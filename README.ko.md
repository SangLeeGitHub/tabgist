...
- **7개 빌트인 프리셋** — 인기 프로바이더 원클릭 설정
- **8개 출력 언어** — 자동(페이지 언어 따라감), 한국어, English, 日本語, 中文, Español, Français, Deutsch
- **다크 모드** — 시스템 설정에 자동 연동
- **연결 테스트** — API 설정 저장 전 작동 여부 확인
- **Thinking 타이머** — 첫 토큰 대기 중 실시간 경과 시간 표시
- **통계 표시** — 요약 완료 후 Thinking time, 전체 시간, t/s 표시
- **잘림 경고** — max_tokens 도달로 출력이 잘린 경우 자동 알림
- **커스텀 시스템 프롬프트** — 기본 프롬프트를 덮어써 출력 형식 커스터마이징
- **`max_completion_tokens` 옵션** — `max_tokens` 대신 `max_completion_tokens`를 사용하는 모델 지원
...
4. 필요에 따라 **Base URL**, **Model**, **Max Tokens**, **System Prompt** 조정 — 프리셋 드롭다운 활용 가능
5. (선택) 일부 모델(o1, o3, gpt-5.4-nono 등)에 필요한 **Use `max_completion_tokens`** 활성화
6. **Test Connection** 클릭하여 확인
7. **Save** 클릭
...
- 토큰 한도에 도달해 출력이 잘린 경우 자동으로 경고가 표시됩니다
